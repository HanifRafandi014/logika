    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="filter-group">
            <label for="categoryFilter">Filter Mata Pelajaran:</label>
                    ...
                    <td>
                        <div class="action-buttons">
                            @php
                                ...
                                $nilaiAkademik = null;
                                if ($selectedCategory) {
                                    $nilaiAkademik = $existingScoresMap->get($siswa->id);
                                    $hasScore = ($nilaiAkademik !== null);
                                }
                            @endphp

                            {{-- Plus button (Always active if category selected, disabled otherwise) --}}
                            <a href="{{ route('nilai_akademik.create', ['mata_pelajaran' => $selectedCategory]) }}"
                               class="btn btn-sm btn-primary"
                               title="{{ $selectedCategory ? 'Tambah Nilai Akademik' : 'Pilih mata pelajaran terlebih dahulu untuk menambah nilai' }}"
                            ...
                            <form action="{{ $hasScore ? route('nilai_akademik.destroy', $nilaiAkademik->id) : '#' }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="btn btn-sm btn-danger"
                                        onclick="return confirm('Yakin ingin menghapus nilai ini?')"
                                        title="{{ $selectedCategory ? ($hasScore ? 'Hapus Nilai' : 'Belum ada nilai untuk mata pelajaran ini') : 'Pilih mata pelajaran terlebih dahulu untuk menghapus nilai' }}"
                                        @if (!$selectedCategory || !$hasScore) disabled @endif>
    ...
    $(document).ready(function () {
        ...
        $('#categoryFilter').on('change', function() {
            ...
            if (selectedCategory) {
                ...
                window.location.href = "{{ route('nilai_akademik.index') }}?mata_pelajaran=" + encodeURIComponent(selectedCategory);
            } else {
                // If "Pilih Kategori" is selected, just redirect to index without category
                window.location.href = "{{ route('nilai_akademik.index') }}";
            }
        });
    });
</script>