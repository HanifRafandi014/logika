...
<div class="col-md-12">
    ...
    <div class="table-container">
        <h4 class="table-title" style="text-align: center;">Tabel Nilai Akademik Siswa</h4>
        ...
        @if ($academicGrades->isEmpty())
            <p class="no-data-message">Tidak ada data nilai akademik untuk siswa ini pada semester yang dipilih.</p>
        @else
            <div class="table-responsive">
                <table id="multi-filter-select-academic" class="table table-striped table-hover">
                    <thead>
                        <tr>
                            <th>No</th>
                    ...
                    <tbody>
                        @foreach ($academicGrades as $index => $nilai)
                            <tr>
                                <td>{{ $index + 1 }}</td>
...
<script>
    $(document).ready(function () {
        // Inisialisasi DataTable untuk tabel akademik
        $('#multi-filter-select-academic').DataTable({
            orderCellsTop: true,
            fixedHeader: true,
            pageLength: 5
        });
