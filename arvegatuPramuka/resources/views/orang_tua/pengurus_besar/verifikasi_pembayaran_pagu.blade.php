{{-- resources/views/orang_tua/pengurus_besar/verifikasi_pembayaran_pagu.blade.php --}}
@extends('layouts.main') {{-- Sesuaikan dengan layout utama Anda --}}
@section('sidebar')
    @include('layouts.sidebar.orang_tua') {{-- Sesuaikan dengan sidebar Pengurus Paguyuban Besar --}}
@endsection
...
<style>
    ...
    .detail-label {
        font-weight: bold;
        color: #666;
        width: 170px; /* Adjusted width for labels */
        flex-shrink: 0;
    }
    .detail-value {
        color: #333;
...
<div class="col-md-12">
    <div class="header-section">
        ...
        <h4 class="page-title">
            Verifikasi Pembayaran Paguyuban
            @if(isset($filteredPengurus))
                dari Pengurus {{ $filteredPengurus }}
            @endif
        </h4>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <p class="mb-3">Nama Pengurus Besar : <strong>{{ $loggedInUser->orang_tua->nama ?? 'N/A' }}</strong></p>

            @forelse ($setoranUntukVerifikasi as $setoran)
                <div class="payment-item-card">
                    <div class="payment-row-detail">
                        <span class="detail-label">Kelas :</span>
                        <span class="detail-value">{{ $setoran->kelas ?? 'N/A' }}</span>
                    </div>
                    <div class="payment-row-detail">
                        <span class="detail-label">Nama Pengurus Kelas :</span>
                        <span class="detail-value">{{ $setoran->pengurus_kelas->nama ?? 'N/A' }}</span>
                    </div>
                    <div class="payment-row-detail">
                        <span class="detail-label">Bulan :</span>
                        <span class="detail-value">
                            @if ($setoran->bulan_setor)
                                {{ implode(', ', $setoran->bulan_setor) }}
                            @else
                                N/A
                            @endif
                        </span>
                    ...
                    <div class="payment-row-detail">
                        ...
                        <span class="detail-value detail-value-status left-align">
                            @if ($setoran->status_verifikasi == 0)
                    ...
                    <div class="payment-row-detail">
                        ...
                        <span class="detail-value action-buttons left-align">
                            @if ($setoran->bukti_setor)
                                <a href="#" class="btn btn-action-custom bukti" data-bs-toggle="modal" data-bs-target="#buktiSetorModal{{ $setoran->id }}">
                                    Lihat Bukti
                                </a>
                                <div class="modal fade" id="buktiSetorModal{{ $setoran->id }}" tabindex="-1" aria-labelledby="buktiSetorModalLabel{{ $setoran->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="buktiSetorModalLabel{{ $setoran->id }}">Bukti Pembayaran untuk Kelas {{ $setoran->kelas }}</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <img src="{{ Storage::url($setoran->bukti_setor) }}" alt="Bukti Setoran" class="img-fluid">
                                            </div>
                            ...
                            @if ($setoran->status_verifikasi == 0)
                                <form action="{{ route('pembayaran-setoran.verify', $setoran->id) }}" method="POST" class="verify-form">
                                    @csrf
                                    <button type="button" class="btn btn-action-custom verify verify-button" data-setoran-id="{{ $setoran->id }}">Verifikasi</button>
                                </form>
                            @else
            ...
            @empty
                <p class="text-center py-4">Tidak ada pembayaran yang perlu diverifikasi.</p>
            @endforelse
        </div>
    </div>
</div>
